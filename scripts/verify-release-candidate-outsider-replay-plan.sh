...
paths = [
    "CINEMATICUM_RELEASE_CANDIDATE_OUTSIDER_REPLAY_PLAN.json",
    "CINEMATICUM_RELEASE_CANDIDATE_OUTSIDER_REPLAY_PLAN_LAW.json",
    f"CASES/{CASE_ID}/RELEASE_CANDIDATE_OUTSIDER_REPLAY_PLAN/RELEASE_CANDIDATE_OUTSIDER_REPLAY_PLAN.json",
    f"CASES/{CASE_ID}/RELEASE_CANDIDATE_OUTSIDER_REPLAY_PLAN_STATUS.json",
]
...
for obj in objs:
    ...
    assert obj["release_candidate_outsider_replay_plan_sealed"] is True
    ...
    assert obj["outsider_replay_plan_private_access_required"] is False
    assert obj["outsider_replay_plan_media_or_model_payload_required"] is False
    assert obj["outsider_replay_plan_network_required_after_clone"] is False
    assert obj["outsider_replay_execution_record_present"] is False
    assert obj["outsider_replay_passage_record_present"] is False
    assert obj["required_remaining_release_candidate_gap_count"] == 1
    ...
    assert obj["outsider_replay_plan_does_not_execute_replay"] is True
    assert obj["outsider_replay_plan_does_not_pass_outsider_replay"] is True
    assert obj["outsider_replay_plan_does_not_create_admissibility_verdict"] is True
    assert obj["outsider_replay_plan_does_not_create_terminal_closure"] is True
    assert obj["outsider_replay_plan_does_not_create_release_candidate"] is True
    assert obj["outsider_replay_plan_does_not_issue_motion_picture"] is True
    assert obj["outsider_replay_plan_does_not_admit_media"] is True
    assert obj["outsider_replay_plan_does_not_mutate_current_state"] is True
    assert obj["next_required_object"] == NEXT

record = objs[2]
assert record["record_id"] == "OUTSIDER_REPLAY_PLAN_001_RELEASE_CANDIDATE"

print("CINEMATICUM RELEASE CANDIDATE OUTSIDER REPLAY PLAN: PASS")
...
print("OUTSIDER_REPLAY_PLAN_DECLARED=true")
...
print("REQUIRED_REMAINING_RELEASE_CANDIDATE_GAP_COUNT=1")
