...
surface = load("CINEMATICUM_AUTHORITY_OBJECT_ADMISSION_REQUEST_VALIDATOR.json")
...
assert surface["object_type"] == "CINEMATICUM_AUTHORITY_OBJECT_ADMISSION_REQUEST_VALIDATOR"
...
for obj in (surface, law, status):
    ...
    assert obj['admission_request_validation_passed'] is True
