...
NEXT = "RELEASE_CANDIDATE_PUBLIC_INSPECTION_DOSSIER"

paths = [
    "CINEMATICUM_RELEASE_CANDIDATE_EVIDENCE_BUNDLE.json",
    "CINEMATICUM_RELEASE_CANDIDATE_EVIDENCE_BUNDLE_LAW.json",
    f"CASES/{CASE_ID}/RELEASE_CANDIDATE_EVIDENCE_BUNDLE/RELEASE_CANDIDATE_EVIDENCE_BUNDLE.json",
    f"CASES/{CASE_ID}/RELEASE_CANDIDATE_EVIDENCE_BUNDLE_STATUS.json",
]
...
for obj in objs:
    ...
    assert obj["release_candidate_evidence_bundle_sealed"] is True
    ...
    assert obj["bundled_release_candidate_evidence_artifact_count"] == 3
    assert obj["required_remaining_release_candidate_gap_count"] == 3
    ...
    assert obj["evidence_bundle_does_not_create_release_candidate"] is True
    assert obj["evidence_bundle_does_not_create_public_inspection_dossier"] is True
    assert obj["evidence_bundle_does_not_create_outsider_replay_plan"] is True
    assert obj["evidence_bundle_does_not_create_terminal_closure"] is True
    assert obj["evidence_bundle_does_not_pass_outsider_replay"] is True
    assert obj["evidence_bundle_does_not_issue_motion_picture"] is True
    assert obj["evidence_bundle_does_not_admit_media"] is True
    assert obj["evidence_bundle_does_not_mutate_current_state"] is True
    assert obj["authority_satisfied"] is False
    assert obj["may_advance_now"] is False
    assert obj["next_required_object"] == NEXT

record = objs[2]
assert record["record_id"] == "EVIDENCE_BUNDLE_001_RELEASE_CANDIDATE"

print("CINEMATICUM RELEASE CANDIDATE EVIDENCE BUNDLE: PASS")
...
print("REQUIRED_REMAINING_RELEASE_CANDIDATE_GAP_COUNT=3")
