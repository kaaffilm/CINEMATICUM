...
validator_status = load("CASES/CASE_001_THE_LAST_RENDER/AUTHORITY_OBJECT_ADMISSION_INTAKE_REOPENING_REQUEST_VALIDATOR_STATUS.json")
index = load("CINEMATICUM_CURRENT_STATE_INDEX.json")
case = load("CASES/CASE_001_THE_LAST_RENDER/CURRENT_CASE_STATE.json")
registry = load("CINEMATICUM_OBJECT_REGISTRY.json")

for obj in (corpus, law, status):
    assert obj["case_id"] == CASE_ID
    assert obj["current_state"] == TARGET

    assert obj["authority_object_admission_intake_reopening_request_rejection_corpus_passed"] is True
    assert obj["reopening_request_rejection_corpus_present"] is True
    assert obj["reopening_request_rejection_corpus_sealed"] is True
    assert obj["reopening_request_rejection_corpus_non_authoritative"] is True
    ...
    assert obj["corpus_does_not_satisfy_authority_objects"] is True
    assert obj["corpus_does_not_reopen_current_snapshot"] is True
    assert obj["corpus_does_not_create_new_snapshot"] is True
    assert obj["corpus_does_not_accept_requests"] is True
    assert obj["corpus_does_not_reject_live_requests"] is True
    ...
    assert obj["fixture_count"] == len(FIXTURES)
    assert obj["request_file_pattern"] == "fixtures/AUTHORITY_OBJECT_ADMISSION_INTAKE_REOPENING_REQUEST_REJECTION_CORPUS/*.json"

    for reason in FIXTURES.values():
        assert reason in obj["canonical_rejection_reasons"]
...
for filename, expected_reason in FIXTURES.items():
    fixture = load("fixtures/AUTHORITY_OBJECT_ADMISSION_INTAKE_REOPENING_REQUEST_REJECTION_CORPUS/" + filename)
    assert fixture["fixture"] is True
    assert fixture["live_request"] is False
    assert fixture["current_state"] == TARGET
    assert fixture["expected_rejection_reason"] == expected_reason
    assert fixture["rejection_reason"] == expected_reason
    assert fixture["accepted"] is False
    assert fixture["valid"] is False
    assert fixture["rejected"] is True
    assert fixture["release_candidate_ready"] is False
    assert fixture["authority_satisfied"] is False
    assert fixture["may_advance_now"] is False
    assert fixture["issuance_unblocked"] is False
    assert fixture["issued"] is False

assert validator_status["current_state"] == TARGET
assert validator_status["reopening_request_validator_present"] is True
assert validator_status["validator_non_authoritative"] is True
...
print("CINEMATICUM AUTHORITY OBJECT ADMISSION INTAKE REOPENING REQUEST REJECTION CORPUS: PASS")
print("CURRENT_STATE=" + TARGET)
print("REQUEST_FILE_PATTERN=fixtures/AUTHORITY_OBJECT_ADMISSION_INTAKE_REOPENING_REQUEST_REJECTION_CORPUS/*.json")
