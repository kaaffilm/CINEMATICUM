...
def cmd_runtime_operation_check(args):
    ok, missing = validate_runtime_operation_ready(args.case_id)
    print(json.dumps({
        "case_id": args.case_id,
        "runtime_operation_ready": ok,
        "missing": missing,
    }, indent=2))


def cmd_observability_check(args):
    ok, missing = validate_observability_ready(args.case_id)
    print(json.dumps({
        "case_id": args.case_id,
        "observability_ready": ok,
        "missing": missing,
    }, indent=2))
...
def cmd_recovery_verification_check(args):
    ok, missing = validate_recovery_verification_ready(args.case_id)
    print(json.dumps({
        "case_id": args.case_id,
        "recovery_verification_ready": ok,
        "missing": missing,
    }, indent=2))
...
def main() -> None:
    ...
    p = sub.add_parser("runtime-operation-check")
    p.add_argument("case_id")
    p.set_defaults(func=cmd_runtime_operation_check)
    ...
    p = sub.add_parser("recovery-verification-check")
    p.add_argument("case_id")
    p.set_defaults(func=cmd_recovery_verification_check)
