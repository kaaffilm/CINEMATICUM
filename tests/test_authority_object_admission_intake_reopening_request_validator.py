...
class TestAuthorityObjectAdmissionIntakeReopeningRequestValidator(unittest.TestCase):
    def test_status_contract(self):
        status = json.loads(Path("CASES/CASE_001_THE_LAST_RENDER/AUTHORITY_OBJECT_ADMISSION_INTAKE_REOPENING_REQUEST_VALIDATOR_STATUS.json").read_text())
        self.assertEqual(status["current_state"], TARGET)
        self.assertTrue(status["reopening_request_validator_present"])
        self.assertTrue(status["reopening_request_validation_passed"])
    ...
    def test_verifier_passes(self):
        ...
        self.assertIn("CINEMATICUM AUTHORITY OBJECT ADMISSION INTAKE REOPENING REQUEST VALIDATOR: PASS", out)
