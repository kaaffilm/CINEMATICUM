...
class TestAuthorityObjectAdmissionIntakeRejectionLedger(unittest.TestCase):
    def test_status_contract(self):
        status = json.loads(Path("CASES/CASE_001_THE_LAST_RENDER/AUTHORITY_OBJECT_ADMISSION_INTAKE_REJECTION_LEDGER_STATUS.json").read_text())
        self.assertEqual(status["current_state"], TARGET)
        self.assertTrue(status["authority_object_admission_intake_rejection_ledger_passed"])
        self.assertTrue(status["intake_rejection_ledger_passed"])
        ...
        self.assertEqual(status["rejected_admission_request_count"], 0)
        self.assertEqual(status["accepted_admission_request_count"], 0)
        self.assertEqual(status["live_admission_request_count"], 0)
        self.assertTrue(status["rejection_ledger_non_authoritative"])
    ...
    def test_verifier_passes(self):
        ...
        self.assertIn("CINEMATICUM AUTHORITY OBJECT ADMISSION INTAKE REJECTION LEDGER: PASS", out)
