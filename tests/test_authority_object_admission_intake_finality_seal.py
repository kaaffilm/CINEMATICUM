...
class TestAuthorityObjectAdmissionIntakeFinalitySeal(unittest.TestCase):
    def test_status_contract(self):
        status = json.loads(Path("CASES/CASE_001_THE_LAST_RENDER/AUTHORITY_OBJECT_ADMISSION_INTAKE_FINALITY_SEAL_STATUS.json").read_text())
        self.assertEqual(status["current_state"], TARGET)
        self.assertTrue(status["authority_object_admission_intake_finality_seal_passed"])
        self.assertTrue(status["intake_finality_seal_passed"])
        self.assertTrue(status["intake_finalized"])
        ...
        self.assertTrue(status["finality_seal_non_authoritative"])
    ...
    def test_verifier_passes(self):
        ...
        self.assertIn("CINEMATICUM AUTHORITY OBJECT ADMISSION INTAKE FINALITY SEAL: PASS", out)
