...
class TestAuthorityObjectAdmissionRequestRejectionCorpus(unittest.TestCase):
    def test_contract(self):
        data = json.loads(Path("CINEMATICUM_AUTHORITY_OBJECT_ADMISSION_REQUEST_REJECTION_CORPUS.json").read_text())
        self.assertEqual(data["current_state"], TARGET)
        self.assertTrue(data['admission_request_rejection_corpus_present'])
    ...
    def test_verifier_passes(self):
        ...
        self.assertIn("CINEMATICUM AUTHORITY OBJECT ADMISSION REQUEST REJECTION CORPUS: PASS", out)
