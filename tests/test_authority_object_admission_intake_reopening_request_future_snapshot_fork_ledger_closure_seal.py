...
class TestAuthorityObjectAdmissionIntakeReopeningRequestFutureSnapshotForkLedgerClosureSeal(unittest.TestCase):
    def test_verifier_passes(self):
        ...
        self.assertIn(
            "CINEMATICUM AUTHORITY OBJECT ADMISSION INTAKE REOPENING REQUEST FUTURE SNAPSHOT FORK LEDGER CLOSURE SEAL: PASS",
            result.stdout,
        )
        ...
        self.assertIn("NO_NEW_SNAPSHOT_CREATED_NOW=true", result.stdout)
        self.assertIn("CLOSURE_SEAL_DOES_NOT_REOPEN_INTAKE=true", result.stdout)
        self.assertIn("CLOSURE_SEAL_DOES_NOT_OPEN_FUTURE_FORK_GATE=true", result.stdout)
        self.assertIn("CLOSURE_SEAL_DOES_NOT_MUTATE_CURRENT_SNAPSHOT=true", result.stdout)
