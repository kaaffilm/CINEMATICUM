...
NEXT = "RELEASE_CANDIDATE_ADMISSIBILITY_VERDICT_RECORD"

def load(path: str):
    return json.loads((ROOT / path).read_text(encoding="utf-8"))

class TestReleaseCandidateOutsiderReplayPassageRecord(unittest.TestCase):
    def test_passage_record_present_after_execution_record(self):
        obj = load("CINEMATICUM_RELEASE_CANDIDATE_OUTSIDER_REPLAY_PASSAGE_RECORD.json")
        ...
        self.assertTrue(obj["release_candidate_outsider_replay_passage_record_sealed"])
        ...
        self.assertTrue(obj["outsider_replay_passage_record_sealed"])
    ...
    def test_passage_record_does_not_advance_beyond_replay(self):
        obj = load("CINEMATICUM_RELEASE_CANDIDATE_OUTSIDER_REPLAY_PASSAGE_RECORD.json")
        for key in [
        ...
        self.assertTrue(obj["passage_record_is_not_admissibility_verdict_record"])
        self.assertTrue(obj["passage_record_is_not_terminal_closure_record"])
        self.assertTrue(obj["passage_record_does_not_create_admissibility_verdict"])
        self.assertTrue(obj["passage_record_does_not_create_terminal_closure"])
        self.assertTrue(obj["passage_record_does_not_create_release_candidate"])
        self.assertTrue(obj["passage_record_does_not_issue_motion_picture"])
        self.assertTrue(obj["passage_record_does_not_admit_media"])
        self.assertTrue(obj["passage_record_does_not_mutate_current_state"])
    ...
    def test_case_record_identity(self):
        obj = load(f"CASES/{CASE_ID}/RELEASE_CANDIDATE_OUTSIDER_REPLAY_PASSAGE_RECORD/RELEASE_CANDIDATE_OUTSIDER_REPLAY_PASSAGE_RECORD.json")
        self.assertEqual(obj["record_id"], "OUTSIDER_REPLAY_PASSAGE_RECORD_001_RELEASE_CANDIDATE")
        self.assertEqual(obj["object_type"], "CINEMATICUM_RELEASE_CANDIDATE_OUTSIDER_REPLAY_PASSAGE_RECORD")
