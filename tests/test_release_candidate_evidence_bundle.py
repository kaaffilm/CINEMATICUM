...
class TestReleaseCandidateEvidenceBundle(unittest.TestCase):
    def test_evidence_bundle_present_and_bounded(self):
        obj = load("CINEMATICUM_RELEASE_CANDIDATE_EVIDENCE_BUNDLE.json")
        ...
        self.assertTrue(obj["release_candidate_evidence_bundle_sealed"])
        ...
        self.assertEqual(obj["bundled_release_candidate_evidence_artifact_count"], 3)
        self.assertEqual(obj["required_remaining_release_candidate_gap_count"], 3)
        self.assertEqual(obj["next_required_object"], NEXT)

    def test_evidence_bundle_does_not_advance_or_issue(self):
        obj = load("CINEMATICUM_RELEASE_CANDIDATE_EVIDENCE_BUNDLE.json")
        for key in [
    ...
    def test_case_record_identity(self):
        obj = load(f"CASES/{CASE_ID}/RELEASE_CANDIDATE_EVIDENCE_BUNDLE/RELEASE_CANDIDATE_EVIDENCE_BUNDLE.json")
        self.assertEqual(obj["record_id"], "EVIDENCE_BUNDLE_001_RELEASE_CANDIDATE")
        self.assertEqual(obj["object_type"], "CINEMATICUM_RELEASE_CANDIDATE_EVIDENCE_BUNDLE")
