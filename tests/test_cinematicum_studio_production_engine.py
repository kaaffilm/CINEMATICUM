...
def test_take_source_admissibility_grant_issuer_requires_root_authority():
    import hashlib
    ...
    evidence_path = film_dir / "TAKE_SOURCE_ADMISSIBILITY_LEDGER.json"
    payload_dir = film_dir / "SOURCE_ADMISSIBILITY_EVIDENCE"
    payload_path = payload_dir / "SHOT_001_TAKE_001.payload_with_issuer.json"
    authority_path = payload_dir / "SOURCE_AUTHORITY_001.with_issuer.json"
    grant_path = payload_dir / "SOURCE_AUTHORITY_001.grant_with_issuer.json"
    issuer_path = payload_dir / "SOURCE_GRANT_ISSUER_001.rootless.json"
    ...
    authority_backup = authority_path.read_text() if authority_path.exists() else None
    grant_backup = grant_path.read_text() if grant_path.exists() else None
    issuer_backup = issuer_path.read_text() if issuer_path.exists() else None
    ...
    try:
        ...
        issuer_path.write_text(json.dumps({
            "object_type": "CINEMATICUM_TAKE_SOURCE_ADMISSIBILITY_GRANT_ISSUER",
            "case_id": CASE_ID,
        ...
        issuer_sha256 = hashlib.sha256(issuer_path.read_bytes()).hexdigest()
        ...
        grant_path.write_text(json.dumps({
            ...
            "scope": "TAKE_SOURCE_ADMISSIBILITY",
            "revoked": False,
            "grant_issuer_id": "SOURCE_GRANT_ISSUER_001",
            "grant_issuer_record_path": str(issuer_path.relative_to(ROOT)),
            "grant_issuer_record_sha256": issuer_sha256,
        }, indent=2) + "\n")
        ...
        assert ok is False
        assert "TAKE_SOURCE_ADMISSIBILITY_AUTHORITY_GRANT_ISSUER_ROOT_UNBOUND" in missing
    finally:
        ledger_path.write_text(ledger_backup)
...
def test_take_source_case_closure_admission_ledger_must_match_status_mirror():
    import hashlib

    film_dir = ROOT / "CASES" / CASE_ID / "FILM"
    evidence_dir = film_dir / "SOURCE_ADMISSIBILITY_EVIDENCE"
    closure_path = evidence_dir / "ROOT_SOURCE_AUTHORITY_001.case_closure_admission_status_unbound.json"
    admission_path = evidence_dir / "ROOT_SOURCE_AUTHORITY_001.case_closure_admission_status_unbound.decision.json"
    ledger_path = ROOT / "CINEMATICUM_AUTHORITY_OBJECT_ADMISSION_DECISION_LEDGER.json"
    status_path = ROOT / "CASES" / CASE_ID / "AUTHORITY_OBJECT_ADMISSION_DECISION_LEDGER_STATUS.json"

    closure_backup = closure_path.read_text() if closure_path.exists() else None
    admission_backup = admission_path.read_text() if admission_path.exists() else None
    ledger_backup = ledger_path.read_text()
    status_backup = status_path.read_text() if status_path.exists() else None

    closure_id = "ROOT_SOURCE_AUTHORITY_CASE_CLOSURE_STATUS_UNBOUND"
    root_authority_id = "ROOT_SOURCE_AUTHORITY_STATUS_UNBOUND"
    admission_object_id = "ROOT_SOURCE_AUTHORITY_CASE_CLOSURE_STATUS_UNBOUND_ADMISSION"
    ...
    try:
        ...
        ledger["decision_record_count"] = 1
        ledger["admission_decision_count"] = 1
        ledger["accepted_decision_count"] = 1
        ledger["rejected_decision_count"] = 0
        ledger["decision_records_present"] = True
        ledger["accepted_decisions_present"] = True
        ledger["rejected_decisions_present"] = False
        ledger_path.write_text(json.dumps(ledger, indent=2) + "\n")

        # Deliberately leave status mirror at its original zero-state values.
        ok, missing = validate_admissible_motion_picture(CASE_ID)

        assert ok is False
        assert "TAKE_SOURCE_ADMISSIBILITY_CASE_CLOSURE_ADMISSION_LEDGER_STATUS_UNBOUND" in missing
    finally:
        if closure_backup is None:
            closure_path.unlink(missing_ok=True)
        else:
        ...
        if status_backup is None:
            ...
            status_path.write_text(status_backup)

def test_take_source_case_closure_admission_must_be_in_authority_object_docket():
    import hashlib

    film_dir = ROOT / "CASES" / CASE_ID / "FILM"
    evidence_dir = film_dir / "SOURCE_ADMISSIBILITY_EVIDENCE"
    closure_path = evidence_dir / "ROOT_SOURCE_AUTHORITY_001.case_closure_docket_unbound.json"
    admission_path = evidence_dir / "ROOT_SOURCE_AUTHORITY_001.case_closure_docket_unbound.decision.json"
    ledger_path = ROOT / "CINEMATICUM_AUTHORITY_OBJECT_ADMISSION_DECISION_LEDGER.json"
    status_path = ROOT / "CASES" / CASE_ID / "AUTHORITY_OBJECT_ADMISSION_DECISION_LEDGER_STATUS.json"
    ...
    closure_id = "ROOT_SOURCE_AUTHORITY_CASE_CLOSURE_DOCKET_UNBOUND"
    root_authority_id = "ROOT_SOURCE_AUTHORITY_DOCKET_UNBOUND"
    admission_object_id = "ROOT_SOURCE_AUTHORITY_CASE_CLOSURE_DOCKET_UNBOUND_ADMISSION"
    ...
    try:
        ...
        # Deliberately leave the authority-object admission docket without a
        # matching admitted-object trace for this closure admission.
        ok, missing = validate_admissible_motion_picture(CASE_ID)

        assert ok is False
        assert "TAKE_SOURCE_ADMISSIBILITY_CASE_CLOSURE_ADMISSION_DOCKET_UNBOUND" in missing
    finally:
        if closure_backup is None:
            closure_path.unlink(missing_ok=True)
        else:
...
def test_take_source_case_closure_registry_entry_must_bind_object_hash():
    import hashlib

    film_dir = ROOT / "CASES" / CASE_ID / "FILM"
    evidence_dir = film_dir / "SOURCE_ADMISSIBILITY_EVIDENCE"
    closure_path = evidence_dir / "ROOT_SOURCE_AUTHORITY_001.case_closure_registry_hash_unbound.json"
    admission_path = evidence_dir / "ROOT_SOURCE_AUTHORITY_001.case_closure_registry_hash_unbound.decision.json"
    registry_path = ROOT / "CINEMATICUM_OBJECT_REGISTRY.json"
    ...
    closure_id = "ROOT_SOURCE_AUTHORITY_CASE_CLOSURE_REGISTRY_HASH_UNBOUND"
    root_authority_id = "ROOT_SOURCE_AUTHORITY_REGISTRY_HASH_UNBOUND"
    admission_object_id = "ROOT_SOURCE_AUTHORITY_CASE_CLOSURE_REGISTRY_HASH_UNBOUND_ADMISSION"
    ...
    try:
        ...
        assert ok is False
        assert "TAKE_SOURCE_ADMISSIBILITY_CASE_CLOSURE_ADMISSION_REGISTRY_OBJECT_HASH_UNBOUND" in missing
    finally:
        if closure_backup is None:
            closure_path.unlink(missing_ok=True)
        else:
...
def test_take_source_case_closure_registry_entry_must_bind_schema_identity():
    ...
    film_dir = ROOT / "CASES" / CASE_ID / "FILM"
    evidence_dir = film_dir / "SOURCE_ADMISSIBILITY_EVIDENCE"
    closure_path = evidence_dir / "ROOT_SOURCE_AUTHORITY_001.case_closure_registry_schema_unbound.json"
    admission_path = evidence_dir / "ROOT_SOURCE_AUTHORITY_001.case_closure_registry_schema_unbound.decision.json"
    registry_path = ROOT / "CINEMATICUM_OBJECT_REGISTRY.json"
    ...
    closure_id = "ROOT_SOURCE_AUTHORITY_CASE_CLOSURE_REGISTRY_SCHEMA_UNBOUND"
    root_authority_id = "ROOT_SOURCE_AUTHORITY_REGISTRY_SCHEMA_UNBOUND"
    admission_object_id = "ROOT_SOURCE_AUTHORITY_CASE_CLOSURE_REGISTRY_SCHEMA_UNBOUND_ADMISSION"
    admission_object_type = "CINEMATICUM_AUTHORITY_OBJECT_ADMISSION_DECISION"
    ...
    try:
        ...
        admission_path.write_text(json.dumps({
            "object_type": admission_object_type,
        ...
        registry["entries"] = [
            entry for entry in registry["entries"]
            if entry.get("path") != admission_rel
        ]
        registry["entries"].append({
            "path": admission_rel,
