...
list-shots:
>python3 -c 'import json; from pathlib import Path; data=json.loads(Path("production/THE_LAST_RENDER/shots/shotlist.json").read_text()); [print(s.get("file") or s.get("filename") or s.get("source") or s.get("path") or (s.get("id","") + ".mp4")) for s in data.get("shots", data)]'
